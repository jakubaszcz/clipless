...
pub(crate) fn get_clips(conn: &Connection) -> rusqlite::Result<Vec<clipboard::Clipboard>> {
    let mut stmt = conn.prepare("SELECT id, content, timestamp, use_clip FROM clips")?;
...
pub(crate) fn update_use_clip(conn: &Connection, id: u32) -> rusqlite::Result<()> {
    let mut stmt = conn.prepare("UPDATE clips SET use_clip = use_clip + 1 WHERE id = ?1")?;
    stmt.execute([id])?;
    Ok(())
}
...
pub(crate) fn fetch_clips(conn: &Connection, query: &str) -> rusqlite::Result<Vec<clipboard::Clipboard>> {
    let mut stmt = conn.prepare("SELECT id, content, timestamp, use_clip FROM clips WHERE content LIKE ?1 ESCAPE '\\'")?;
